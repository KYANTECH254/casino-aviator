...
export default function AddUser({ onClose }: { onClose: () => void; }) {
    ...
    const AddUserF = async () => {
        if (!balance || !station) {
            addAlert("All fields are required!", 3000, "red", 1, true);
        }
        const data = {
            token: token,
            balance: balance,
        ...
        setLoading(true)
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/addUser`, {
            method: "POST",
            body: JSON.stringify(data),
            headers: { 'Content-Type': 'application/json' },
        })
        ...
        <div id="aviator-avatar-tab" className="aviator-popup-container">
            <div ref={popupRef} className="aviator-avatar-popup">
                <div className="aviator-popup-header">
                    <div className="aviator-popup-header-left">Add User</div>
                ...
                <div className="aviator-avatar-popup-body">
                    <form method="post" className="aviator-form">
                        ...
                        {screens.length === 0 ? (
                            ...
                            <>
                                <select name="Screen" id="Screen" onChange={(e) => setStation(e.target.value)} required>
                        ...
                        <label htmlFor="User Balance">User Balance</label>
                        <input value={balance} onChange={(e) => setBalance(e.target.value)} type="number" name="balance" id="balance" placeholder="Enter amount" />
                        <button disabled={station === "" || balance === ""} onClick={AddUserF} type="button">{loading ? 'Creating...' : 'Add'}</button>
                    </form>
                </div>
            </div>
        </div>
